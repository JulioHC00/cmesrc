...
# flares_data["time_start"] = Time(flares_data["time_peak"])

flares_data = flares_data[flares_data["MATCH"]]
dimmings_data = dimmings_data[dimmings_data["MATCH"]]

dimmings_data["start_time"] = Time(dimmings_data["start_time"].to_list())
flares_data["time_start"] = Time(flares_data["time_start"].to_list())
    ...
    for main_idx, cme in cmes.iterrows():
        ...
        if harpnum in grouped_flares_data.groups:
            ...
            possible_flares = flares[
                (flares["time_start"] >= cme_date - MAX_FLARE_TIME_BEFORE_CME) &
                (flares["time_start"] <= cme_date + MAX_FLARE_TIME_AFTER_CME) &
                (~ flares["hec_id"].isin(matched_flares))
            ].copy()
            ...
            if len(possible_flares) > 0:
                ...
                matched_flares.append(flare["hec_id"])
                ...
                main_database.loc[main_idx, "FLARE_MATCH"] = flare["hec_id"]
                main_database.loc[main_idx, "FLARE_CLASS_FLAG"] = flare["FLARE_CLASS"] >= MIN_FLARE_CLASS
