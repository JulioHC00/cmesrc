...
cur.execute(
    "CREATE INDEX IF NOT EXISTS idx_temp_zarr_index_harpnum_timestamp ON temp_zarr_index (harpnum, timestamp)"
)
cur.execute(
    """
...
new_data = [
    (timestamp, year, month, day, hour, minute, second, idx)
    for timestamp, year, month, day, hour, minute, second, idx in zip(
        timestamps, years, months, days, hours, minutes, seconds, indices
    )
]

cur.executemany(
    "INSERT INTO IMAGES (timestamp, year, month, day, hour, minute, second, idx) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(timestamp) DO NOTHING",
    new_data,
)
conn.commit()
...
WHERE mean_overlap > 5 AND ocurrence_percentage > 5""",
    conn,
)
...
count_df = (
    groups.groupby(["group_id", "verification_score"]).size().reset_index(name="count")
)


# Pivot the table to have verification_scores as columns
pivot_df = (
    count_df.pivot(index="group_id", columns="verification_score", values="count")
    .fillna(0)
    .reset_index()
)
...
result_tuples = [
    (int(t[1]), int(t[2]), int(t[3]), int(t[4]), int(t[5]), int(t[0]))
...
N_SPLITS = 10
splits = {
    i: {"group_ids": [], "totals": {col: 0 for col in df.columns if col != "group_id"}}
    for i in range(1, N_SPLITS + 1)
}

# Sort by each n_level_x column in descending order. We sort first level1, level2 ... until at the end group_size
df.sort_values(
    by=[col for col in df.columns if col != "group_id"], ascending=False, inplace=True
)
...
for attribute in attributes:
    ...
    for group_id, row in assign_df.iterrows():
        ...
        sorted_splits = sorted(
            splits.keys(), key=lambda x: splits[x]["totals"][attribute]
        )
...
for split, split_dict in splits.items():
    ...
    cur.executemany(
        "UPDATE SDOML_GROUPS SET split = ?, subsplit = ? WHERE group_id = ?",
        [(split_number, subsplit_number, group_id) for group_id in group_ids],
    )
...
n_rows_pivot = pd.pivot_table(
    nrows,
    values="counts",
    index=["split", "subsplit"],
    columns=["label"],
    aggfunc=np.sum,
)
...
ncmes = (
    df[df["label"].astype(bool)]
    .drop_duplicates(subset=["cme_id"])
    .groupby(["split", "subsplit", "verification_level"])
    .size()
    .reset_index(name="counts")
)

ncmes_pivot = pd.pivot_table(
    ncmes,
    values="counts",
    index=["split", "subsplit"],
    columns=["verification_level"],
    aggfunc=np.sum,
)
...
nreg = (
    ...
    .drop_duplicates(subset=["harpnum"], keep="first")
    .groupby(["split", "subsplit", "label"])
    .size()
    .reset_index(name="counts")
)

nreg_pivot = pd.pivot_table(
    nreg,
    values="counts",
    index=["split", "subsplit"],
    columns=["label"],
